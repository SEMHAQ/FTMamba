#!/bin/bash
# FTMamba + all baselines on Traffic dataset
# Note: Traffic has 862 variates — reduce batch size for memory
set -e
...
echo "===== Traffic Dataset (862 variates) ====="
for model in "${MODELS[@]}"; do
  for pred_len in ${PRED_LENS}; do
    echo "Running: ${model} on Traffic (pred_len=${pred_len})"
