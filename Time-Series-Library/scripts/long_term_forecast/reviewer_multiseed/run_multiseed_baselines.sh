#!/bin/bash
# Multi-seed evaluation for PatchTST (3 seeds: 2021, 42, 1234)
# to enable symmetric multi-seed comparison with FTMamba
set -e
...
# Only run PatchTST (primary competitor) with 3 seeds
# Other baselines can be added if time permits
MODELS=("PatchTST")

for dataset in "${DATASETS[@]}"; do
  for model in "${MODELS[@]}"; do
    for pred_len in ${PRED_LENS}; do
      echo "Running: ${model} on ${dataset} (pred_len=${pred_len}, itr=3)"
...
echo "Multi-seed baseline experiments completed!"
