#!/bin/bash
# FTMamba + all baselines on Electricity dataset
set -e
...
echo "===== Electricity Dataset (321 variates) ====="
for model in "${MODELS[@]}"; do
  for pred_len in ${PRED_LENS}; do
    echo "Running: ${model} on Electricity (pred_len=${pred_len})"
    ...
    python -u run.py \
      ...
      --model_id Electricity_${pred_len}_${pred_len} \
      --model $model \
      --data custom \
      --features M \
      --seq_len ${SEQ_LEN} \
